...
circle.Model = class {
    ...
    constructor(metadata, model) {
        ...
        for (const key of Object.keys(circle.schema.BuiltinOperator)) {
            ...
            builtinOperatorMap.set(index, circle.Utility.type(key));
        }
        for (let i = 0; i < model.operator_codes.length; i++) {
            const operatorCode = model.operator_codes[i];
            const code = operatorCode.deprecated_builtin_code < circle.schema.BuiltinOperator.PLACEHOLDER_FOR_GREATER_OP_CODES ? operatorCode.deprecated_builtin_code : operatorCode.builtin_code;
            ...
            if (!name) {
                throw new circle.Error("Invalid built-in code '" + code.toString() + "' at '" + i.toString() + "'.");
            }
        ...
        for (let i = 0; i < subgraphs.length; i++) {
            ...
            this._graphs.push(new circle.Graph(metadata, subgraph, subgraphMetadata, name, operatorList, model));
        }
    }
...
circle.Graph = class {
    ...
    constructor(metadata, subgraph, subgraphMetadata, name, operatorList, model) {
        ...
        for (let i = 0; i < subgraph.tensors.length; i++) {
            ...
            const initializer = (data && data.length > 0) || is_variable ? new circle.Tensor(i, tensor, buffer, is_variable) : null;
            args.push(new circle.Argument(i, tensor, initializer));
        ...
        for (let i = 0; i < inputs.length; i++) {
            ...
            this._inputs.push(new circle.Parameter(tensorNames[input], true, [ argument ]));
        }
        ...
        for (let i = 0; i < outputs.length; i++) {
            ...
            this._outputs.push(new circle.Parameter(tensorNames[output], true, [ argument ]));
        }
    }
    ...
    constructor(metadata, node, type, location, args) {
        ...
        if (node) {
            ...
            for (let k = 0; k < outputs.length; k++) {
                ...
                this._outputs.push(new circle.Parameter(outputName, true, [ argument ]));
            }
                ...
                if (node.custom_options_format === circle.schema.CustomOptionsFormat.FLEXBUFFERS) {
                    try {
                        ...
                        for (const key of Object.keys(custom_options)) {
                            ...
                            this._attributes.push(new circle.Attribute(schema, key, value));
                        }
                        decoded = true;
                    }
                    catch (err) {
    ...
    constructor(schema, name, value) {
        ...
        if (schema) {
            ...
            if (this._type) {
                switch (this._type) {
                    case 'shape':
                        this._value = new circle.TensorShape(value);
                        break;
                    case 'TensorType':
                        this._value = circle.Utility.dataType(this._value);
                        break;
                    default:
...
circle.Utility = class {
    ...
    static dataType(type) {
        if (!circle.Utility._tensorTypeMap) {
            circle.Utility._tensorTypeMap = new Map();
            for (const name of Object.keys(circle.schema.TensorType)) {
                circle.Utility._tensorTypeMap.set(circle.schema.TensorType[name], name.toLowerCase());
            }
    ...
    static enum(name, value) {
        ...
        if (type) {
            circle.Utility._enumKeyMap = circle.Utility._enumKeyMap || new Map();
            if (!circle.Utility._enumKeyMap.has(name)) {
                const map = new Map();
                for (const key of Object.keys(type)) {
                    map.set(type[key], key);
                }
                circle.Utility._enumKeyMap.set(name, map);
            }
            const map = circle.Utility._enumKeyMap.get(name);
            if (map.has(value)) {
...
circle.Error = class extends Error {
    ...
    constructor(message) {
        super(message);
        this.name = 'Error loading ONE CIRCLE model.';
    }
};
